...
public class Eventv2Controller implements Initializable {
    ...
    @FXML
    private JFXButton inscriButton;
    @FXML
    ...
    @FXML
    private void tiSinscrireForm(ActionEvent event) {
    ...
    public void setLikeIconColor(){
        ...
        this.notifNumber1.setText(String.valueOf(nbrShare));
    }
    @FXML
    ...
    @FXML
    private void shareEvent(MouseEvent event) {
        FXMLLoader Loader=new FXMLLoader();
        Loader.setLocation(getClass().getResource("/views/share.fxml"));
        try {
            Loader.load();  
        } catch (IOException ex) {
            Logger.getLogger(Eventv2Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
        
               ShareController sc=Loader.getController();
                sc.c=this.c;
                sc.e=this.event;
                Parent p=Loader.getRoot();
                Stage share=new Stage();
                Scene scene = new Scene(p);
                share.initStyle(StageStyle.TRANSPARENT);
                share.setScene(scene);
                share.show();
    }
